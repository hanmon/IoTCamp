/*
  ...
  Components:
  1.Buzzer on Pin4,pin4接到Buzzer正極
  2.Push button on Pin5~7 for selecting one of three songs
  Author:Bruce Chiu
  Date: 2019-07-22
  Email:boo.chiu@gmail.com
*/
const int buzzerPin = 4; // pin4接到Buzzer正極
const int btnPins[] = {2, 3, 18}; //pin2,3,18接到按鍵
...
int songNumber = 0;
volatile int btnNumberPushed = 0;
...
{ //無須設定pin模式
  Serial.begin(115200);
  attachInterrupt(digitalPinToInterrupt(2), btn1ISR, RISING);
  attachInterrupt(digitalPinToInterrupt(3), btn2ISR, RISING);
...
void loop()
{
  if (btnNumberPushed != 0) {
    ...
    for (int i = 0; i < songLen; i++)// 根據簡譜播放音樂
    {
      ...
      duration = ((int)(tempoMap[songNumber][i] - '0')) * 1000 / 4;
